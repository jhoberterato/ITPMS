...
import { modalActions, errorDialogActions, snackBarActions, pcActions } from '../../redux/actions';
...
const DeleteForm = () => {
    ...
    const deleteID = useSelector((state) => state.pc.deleteID)
    ...
    const handleDelete = async () => {
        const del = await window.$post('pc/deletePC', {id: deleteID})
        if(del.data.status === "success"){
            dispatch(pcActions.setDeleteID(0))
